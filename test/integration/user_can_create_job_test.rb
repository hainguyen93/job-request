...
class UserCanCreateJobTest < ActionDispatch::IntegrationTest
   def setup
    @staff = users(:staff)
  end


  test "user creates new job with valid info" do
    log_in_as(@staff) 
    get new_job_path
    assert_template 'jobs/new'
    assert_select 'h1', text: 'Create New Job'
    assert_difference 'Job.count', 1 do
      post jobs_path, job: { title: 'job title test',
                             'deadline(1i)': 2015,
                             'deadline(2i)': 6,
                             'deadline(3i)': 1,
                             'deadline(4i)': 12,
    ...
    @job = Job.find_by(title: 'job title test')
